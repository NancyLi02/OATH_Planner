...
def compute_distance(p1, p2):
    ...
    num_samples = max(int(line.length / 0.1), 1)
    ...
    return cost_along_path / num_samples + manhattan
...
# ---------- 修改 Greys colormap ----------
orig_greys = plt.get_cmap("Greys")
modified_greys = mcolors.LinearSegmentedColormap.from_list("ModifiedGreys", orig_greys(np.linspace(0.3, 1, 256)))
...
sc = plt.scatter(sample_points_np[:, 0], sample_points_np[:, 1],
                 c=sample_values_np, cmap=modified_greys, s=10, marker='s', zorder=1)
cbar = plt.colorbar(sc, label='Cost')

# 设置 colorbar 的标签字体大小
cbar.set_label('Cost', fontsize=14)
...
for cluster_label in range(num_clusters_other):
    ...
    # 仅在第一个聚类上设置标签
    label_val = 'A tasks' if cluster_label == 0 else None
    plt.scatter(cluster_coords[:, 0], cluster_coords[:, 1], s=100, c='blue', marker='o',
                edgecolors='blue', linewidths=2, label=label_val, zorder=4)
    center = cluster_coords.mean(axis=0)
    radius = np.max(np.linalg.norm(cluster_coords - center, axis=1)) + 0.4
    # circle = patches.Circle(center, radius, facecolor='lightblue', edgecolor='black', alpha=0.4, zorder=3)
    circle = patches.Circle(center, radius, facecolor='lightblue', edgecolor='blue', alpha=0.4, zorder=3)
    ax.add_patch(circle)
...
for cluster_label in range(num_clusters_special):
    ...
    label_val = 'B tasks' if cluster_label == 0 else None
    plt.scatter(cluster_coords[:, 0], cluster_coords[:, 1], s=100, c='red', marker='^',
                edgecolors='red', linewidths=2, label=label_val, zorder=4)
    center = cluster_coords.mean(axis=0)
    radius = np.max(np.linalg.norm(cluster_coords - center, axis=1)) + 0.4
    circle = patches.Circle(center, radius, facecolor='lightcoral', edgecolor='red', alpha=0.4, zorder=3)
    ax.add_patch(circle)
...
# 固定图例在地图左上角
plt.legend(loc='upper left', bbox_to_anchor=(0, 1), fontsize=16, framealpha=1.0)
